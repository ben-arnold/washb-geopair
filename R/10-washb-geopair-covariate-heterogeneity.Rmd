...
# into the /geodata directory
#---------------------------
ken_elev <- raster::getData("alt", 
                            country="Kenya",
                            path=paste0(Box_geodata_directory, "ken_msk_alt")
                            )

#---------------------------
# mask the elevation to the
# study counties
#---------------------------
ken_elev <- raster::mask(ken_elev, mask = as(ken_adm_sf,"Spatial"))
...
dcl_k$elev <- extract(x = ken_elev, y = dcl_k[,c("lon","lat")])
...
## Create an elevation figure

```{r kenya elevation map}
#--------------------------------------
# convert elevation surface
# raster to a tibble for ggplot
#--------------------------------------
ken_elev2 <- malariaAtlas::as.MAPraster(ken_elev)

# for visualization, recode elevations over 2200 m
# to 2200.
table(ken_elev2$z[is.finite(ken_elev2$z)]>2200)
ken_elev2$z[is.finite(ken_elev2$z) & ken_elev2$z>2200 ] <- 2200


map_elev <- ggplot(data=ken_elev2) + 
  geom_tile(aes(x=x,y=y,fill=z), na.rm=TRUE) +
  ...
  scale_fill_viridis(option = "B", direction = 1,
                     limits = c(1000,2200),
                        na.value = NA,
                        breaks = seq(1000,2200,by=200),
                     labels = c(seq(1000,2000,by=200),"2200+"),
...
```{r plot ascaris prev vs elev, message = FALSE}

plot_alprev_elev <- ggplot(data=dcl_al, aes(x = elev,color = elev)) +
  geom_smooth(aes(y=Y0bar*100),method = "loess", color="gray20", fill="gray20",lwd=0.5,alpha=0.2) +
  geom_point(aes(y = Y0bar*100)) +
  ...
  coord_cartesian(xlim=c(1200,1800),ylim=c(0,80)) +
  labs(x = "elevation (m)", 
...
```{r plot ascaris diff vs elev, message = FALSE}

plot_aldiff_elev <- ggplot(data=dcl_al, aes(x = elev,color = elev)) +
  geom_hline(yintercept = 0, color = "gray50") +
  geom_smooth(aes(y=diff*100),method = "loess", color="gray20", fill="gray20",lwd=0.5,alpha=0.2) +
  geom_point(aes(y = diff*100)) +
  scale_color_viridis(option = "B", direction = 1,
...
```{r elev composite map, message = FALSE, fig.width=4, fig.height = 8}
#--------------------------------------
# create a 1 x 3 composite figure a-c
# having weird difficulty scaling the
# map vis-a-vis scatter plots so 
# saving as separate .png files
#--------------------------------------
ggsave(filename=here("output","washb-geopair-map-elev-ken.png"),plot = map_elev,device="png",width=5.6,height=6.5)

elev_map_composite <- grid.arrange(plot_alprev_elev,
                                  plot_aldiff_elev,
                                  nrow=2,ncol=1,heights=c(1,1))
ggsave(filename=here("output","washb-geopair-al-elev-ken.png"),plot = elev_map_composite,device="png",width=3.5,height=8)
```

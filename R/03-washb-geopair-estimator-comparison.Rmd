...
In this analysis, we compare the estimated relative efficiency based on analytic methods with the empirical relative efficiency. The empirical relative efficiency is calculated by comparing the variance of the treatment effect in an analysis that does not account for matching to the variance of the treatment effect using the Imai matched pair estimator. 
...
# sort by relative efficiency predicted by unweighted correlations
orderb <- ate_res %>% 
...
orderb <- all_results %>%
  filter(country == "Bangladesh") %>% 
  mutate(releff = var_imai/var_nomatch) %>%
  arrange(desc(releff)) %>%
  pull(outcome_lab)

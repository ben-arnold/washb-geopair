...
sumtab <- data.frame(
  outcome_group = c(rep("Child growth",4),rep("Child development",3),rep("Infectious disease",5) ),
  outcome = c("Length-for-age z-score","Weight-for-age z-score","Weight-for-height z-score","Head circumference z-score","EASQ Communication z-score","EASQ Gross motor z-score","EASQ Personal-social z-score","Diarrhea","Giardia sp.","Ascaris sp.","Trichuris sp.","Hookworm"),
  cor_b = c(b_cor_laz,b_cor_waz,b_cor_whz,b_cor_hcz,b_cor_com,b_cor_mot,b_cor_ps,b_cor_diar,b_cor_giar,b_cor_al,b_cor_tt,b_cor_hw),
  cor_k = c(k_cor_laz,k_cor_waz,k_cor_whz,k_cor_hcz,k_cor_com,k_cor_mot,k_cor_ps,k_cor_diar,k_cor_giar,k_cor_al,NA,k_cor_hw)
)
sumtab <- sumtab %>%
...
knitr::kable(sumtab %>% select(-outcome_group),
  ...
  group_rows("Infectious disease",8,12) %>%
...
sumtab2 <- sumtab %>%
  mutate(outcome_lab = c("LAZ","WAZ","WHZ","HCZ","EASQ Communication","EASQ Gross motor","EASQ Personal-social","Diarrhea","Giardia sp.","Ascaris sp.","Trichuris sp.","Hookworm")
         )
sumtabplot <- sumtab2 %>%
...
plot_reff <- ggplot(data=sumtabplot, aes(x = corr, y = releff)) +
  ...
  geom_text(aes(label = outcome_lab, color = outcome_group), hjust = c(0,1,1,1,0,1,0,rep(1,5), 0,0,rep(1,10)), nudge_x = c(0.015,-0.01,-0.01,-0.01,0.015,-0.01,0.015,rep(-0.01,5),0.015,0.015,rep(-0.01,10)),size = 2.5) +
  geom_point(aes(color = outcome_group), size = 1.8) + 
  geom_point(pch=21,bg=NA, size=1.8, show.legend = FALSE) +
  # refine aesthetics
...
ggsave(filename = here("output","washb-geopair-relative-efficacy.png"),plot_reff,device = "png", height=4,width=8.5)
```

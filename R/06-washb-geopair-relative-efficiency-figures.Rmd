...
dreffplot2 <- dreffplot %>%
  ...
  # tedious x and y nudge coordinates for labels to optimize display
  mutate(lab_xnudge = case_when(
           outcome_lab2 %in% c("WAZ","LAZ","WHZ","HCZ") & country == "Bangladesh" ~ -0.03,
           ...
           outcome_lab2 == "Hookworm" & country == "Bangladesh" ~ -0.03,
           
           outcome_lab2 %in% c("Diarrhea","Trichuris sp.") & country == "Kenya" ~ 0.025,
           outcome_lab2 %in% c("LAZ","HCZ") & country == "Kenya" ~ -0.03,
           outcome_lab2 == "WHZ" & country == "Kenya" ~ 0.02,
...
obs_v_pred_reff <- estcomp_res %>% 
  ...
  ggplot(aes(x = eff_u, y = rel_eff, color = outcome_group)) +
...
plot_corr <- ggplot(data = dreffplot, aes(x = corr_w, y = corr_u, color = outcome_group)) +
  ...
  scale_y_continuous(breaks=seq(0,1,by=0.1))+
  labs(y = "unweighted correlation", x = "weighted correlation", tag = "a") +
...
Compare the predicted (based on pair-level weighted correlation) versus observed relative efficiency gains.
...
obs_v_pred_wreff_labs <- d_obs_v_pred_reff %>%
  filter(eff_w - rel_eff >0.4) %>%
  mutate(plot_nudge_x = ifelse(country == "Kenya" & outcome_lab %in% c("Ascaris sp.","length-for-age z"),-0.05,0.05),
         plot_hjust = ifelse(country == "Kenya" & outcome_lab %in% c("Ascaris sp.","length-for-age z"),1,0))

  
obs_v_pred_wreff <- 
  ggplot(data=d_obs_v_pred_reff, aes(x = eff_w, y = rel_eff,color = outcome_group)) +
  geom_abline(intercept=0,slope=1,color="gray70") +
  ...
  geom_text(data = obs_v_pred_wreff_labs, 
            ...
            nudge_x = obs_v_pred_wreff_labs$plot_nudge_x, nudge_y=0.01,show.legend = FALSE) +
  # scale_color_brewer(palette = "Dark2",guide=guide_legend(title="outcome group:")) +
  scale_color_manual(values = pcols, guide=guide_legend(title="outcome group:")) +
  scale_x_continuous(breaks=seq(0.8,3.2,by=0.2))+
  scale_y_continuous(breaks=seq(0.8,3.2,by=0.2))+
  coord_cartesian(xlim=c(0.8,3.2),ylim=c(0.8,3.2))+
  facet_wrap(.~country) +
  labs(x = expression(paste("predicted relative efficiency based on weighted correlation, (1-",italic(r),")")^-1),
       y = "observed relative efficiency",
       tag = "b",
       fill = "outcome group") +
  theme_minimal() +
...
plot_reff_icc <- ggplot(data = d_reff_icc, aes(x = icc, y = rel_eff)) +
  ...
  scale_y_continuous(breaks=seq(0.8,2.6,by=0.4))+
...
estcomp_plot_b <-   ggplot(estcomp_res2 %>% filter(country=="Bangladesh")) + 
  ...
  annotate("text",label="subdistrict stratified",y=1.4,x=15, color="gray40") +
...
estcomp_plot_k <-   ggplot(estcomp_res2 %>% filter(country=="Kenya")) + 
  ...
  annotate("text",label="subdistrict stratified",y=1.9,x=13, hjust=1, color="gray40") +
  labs(x = NULL, y = "relative efficiency compared to unmatched analysis",
       tag = "b") +

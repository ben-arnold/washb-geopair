...
# this function is not used below because we are going
# to go with unweighted correlation, implemented in stats::cor()
corx <- function(x,y,w) {
    xw <- x*w
    yw <- y*w
...
cor_boot_b <- foreach(iteri = 1:1000, .combine = rbind) %dopar% {
  ...
  corest <- di %>% group_map( ~ cor(x=.x$Y1bar, y=.x$Y0bar,method="pearson"))

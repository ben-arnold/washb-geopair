...
# summarize sample sizes by outcome

By country, tabulate the number of matched pairs by outcome, then summarize the number of children in each group (control, nutrition) by outcome.  Finally, summarize the distribution of block sizes.

```{r number of clusters by outcome}
table(dcl$outcome_lab,dcl$country)
```

```{r number of children by outcome}

n_by_group <- dcl %>% 
  group_by(country, outcome_lab) %>% 
  summarize(n1sum=sum(n1),n0sum=sum(n0), .groups="drop")

kbl(n_by_group %>% dplyr::select(-country), 
    col.names = c("Outcome","Nutrition","Control"),
    caption = "Number of children included in the analysis by country and outcome",
    format.args = list(big.mark=",")
    ) %>%
  kable_styling(bootstrap_options = "striped") %>%
  pack_rows(start_row = 1, end_row = 14, group_label = "Bangladesh") %>%
  pack_rows(start_row = 15, end_row = 26, group_label = "Kenya")

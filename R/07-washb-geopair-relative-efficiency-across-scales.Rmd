...
cor_geo_b <- foreach(outi = levels(dcl_b$outcome_lab), .combine = rbind) %dopar% {
  ...
  foreach(blockn = unique(dcl_b$block), .combine = rbind) %do% {
    ...
    # (commented out — using unweighted correlation instead 
    #  based on alignment with the empirical efficiency gains observed in the trial)
    # wY1bar <- di$Y1bar*di$w
    # wY0bar <- di$Y0bar*di$w
    # corri <- 2*cov(wY1bar,wY0bar) / ( var(wY1bar) + var(wY0bar) )
    corri <- cor(x=di$Y1bar,y=di$Y0bar, method = "pearson")
    # return results
...
Use a bootstrap resampling approach, resampling pairs with replacement, to estimate the 95% confidence interval for relative efficiency of geographic pair matching with 90 matched pairs. This will enable us to show whether lower efficiency at smaller sample sizes is significantly lower, and not just a function of sampling variability.
...
cor_boot_b <- foreach(iteri = 1:1000, .combine = rbind) %dopar% {
  ...
  corest <- di %>% group_map( ~ cor(x=.x$Y1bar, y=.x$Y0bar))
...
out_labs_print <- cor90ests_b %>%
  ...
  mutate(y_text = 3.2 - outcome_order*0.1) %>% 
  ungroup() %>%
...
plot_reff <- ggplot(data=dreleff_b, aes(x = npairs, y = releff, color = releff_cat, fill = releff_cat, group = outcome_lab)) +
  ...
  labs(x=expression(paste("number of geographically contiguous matched pairs")),
       y=expression(paste("relative efficiency of geographic pair-matching, (1-",italic(r),")")^-1),
       tag = "a"
       ) +
...
ggsave(here("output","washb-geopair-relative-efficiency-geographic-scale-bangladesh.png"), plot_reff, device = "png", width = 8, height = 5*0.82) # height is scaled vis-a-vis kenya plot b/c of shorter Y-axis to keep vertical ticks proportionally spread
```
...
out_labs_print_k <- cor79ests_k %>%
  ...
  arrange(outcome_group, desc(releff79)) %>%
  mutate(outcome_order = seq_along(outcome_lab)) %>%
  mutate(y_text = 3.8 - outcome_order*0.1) %>% 
  ungroup() %>%
  mutate(releff_cat = case_when(
    releff79 <1.2 ~ "<1.2",
    releff79 >= 1.2 & releff79 < 1.4 ~ "1.2 to 1.4",
...
plot_reff_k <- ggplot(data=dreleff_k, aes(x = npairs, y = releff, color = releff_cat, fill = releff_cat, group = outcome_lab)) +
  ...
  scale_y_continuous(breaks=seq(1,3.8,by=0.4))+
  scale_x_continuous(breaks=seq(10,90,by=20)) +
  coord_cartesian(ylim=c(1,3.6), xlim=c(10,90)) +
  labs(x=expression(paste("number of geographically contiguous matched pairs")),
       y=expression(paste("relative efficiency of geographic pair-matching, (1-",italic(r),")")^-1),
       tag = "b"
       ) +

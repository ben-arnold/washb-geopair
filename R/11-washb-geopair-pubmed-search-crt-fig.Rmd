...
```{r make pubmed figure, fig.width=4}
vircols <- viridis(n=9,option="D")
plotyticks <- seq(0,500,by=100)
plot_pubmed_res <- ggplot(
  data = pubmed_res2,
  aes(x=Year)
  ) +
  geom_bar(aes(y=Count),stat = "identity", fill=vircols[2],alpha=0.6, color = NA) + 
  geom_hline(yintercept = plotyticks[-1], color="white") +
  geom_line(aes(y=pred), color = "black") + 
  annotate("text",x=2018,y=450,label="exponential growth",hjust=1) +

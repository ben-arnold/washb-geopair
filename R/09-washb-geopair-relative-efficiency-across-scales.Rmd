...
cor_geo_b <- foreach(outi = levels(dcl_b$outcome_lab), .combine = rbind) %dopar% {
  foreach(npairs = seq(10,90,by=5), .combine = rbind) %:% 
  foreach(booti = 1:200, .combine = rbind) %do% {
    ...
    mindisti <- min(sampi$block_dist[as.numeric(sampi$block_dist)>0])
...
plot_reff <- ggplot(data=dreleff_b, aes(x = npairs, color = outcome_group, fill = outcome_group)) +
  ...
  geom_ribbon(aes(ymin = releff_lb, ymax = releff_ub), alpha=0.2, color = NA) +
...
plot_reff_b <- ggplot(data=dreleff_b, aes(x = npairs, y = releff, color = releff_cat, fill = releff_cat, group = outcome_lab)) +
  ...
  #
  scale_y_continuous(breaks=seq(1,3.0,by=0.4))+
  scale_x_continuous(breaks=seq(10,90,by=20)) +
...
cor_geo_k <- foreach(outi = levels(dcl_k$outcome_lab), .combine = rbind) %dopar% {
  foreach(npairs = c(seq(10,70,by=5),72), .combine = rbind) %:% 
  foreach(booti = 1:200, .combine = rbind) %do% {
